...
interface OllamaCloudModelListItem {
	id: string;
}

async function fetchOllamaCloudModelIds(): Promise<Set<string>> {
	try {
		console.log("Fetching models from Ollama Cloud API...");
		const response = await fetch(`${OLLAMA_CLOUD_BASE_URL}/models`);
		const data = (await response.json()) as { data?: OllamaCloudModelListItem[] };
		const ids = new Set<string>();
		for (const model of data.data ?? []) {
			ids.add(model.id);
		}
		console.log(`Fetched ${ids.size} model IDs from Ollama Cloud`);
		return ids;
	} catch (error) {
...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		const ollamaCloudModelIds = data["ollama-cloud"]?.models ? await fetchOllamaCloudModelIds() : new Set<string>();
		...
		if (data["ollama-cloud"]?.models) {
			for (const [modelId, model] of Object.entries(data["ollama-cloud"].models)) {
				...
				// Ollama Cloud's /v1/models is the source of truth for live availability.
				// Skip models that are not currently exposed by the Ollama Cloud API
				// (e.g. freshly deprecated before models.dev catches up).
				if (ollamaCloudModelIds.size > 0 && !ollamaCloudModelIds.has(modelId)) continue;
