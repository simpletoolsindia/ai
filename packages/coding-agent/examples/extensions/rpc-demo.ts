...
export default function (ai: ExtensionAPI) {
	...
	ai.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "ai RPC Demo (new session)" : "ai RPC Demo");
	...
	ai.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	ai.on("turn_end", async (_event, ctx) => {
	...
	ai.registerCommand("rpc-prefill", {
