...
Ai runs with all permissions by default, but in some cases, you will want to have more control over what directories Pi can write to and which accesses it has.
...
[Gondolin](https://github.com/simpletoolsindiaorg/gondolin) is a local Linux micro-VM.
Use the [example extension](../examples/extensions/gondolin) when you want `ai` on the host but all built-in tools routed into the VM.

Setup:

```bash
cp -R packages/coding-agent/examples/extensions/gondolin ~/.ai/agent/extensions/gondolin
cd ~/.ai/agent/extensions/gondolin
...
```bash
cd /path/to/project
pi -e ~/.ai/agent/extensions/gondolin
```
...
Requirements: Node.js >= 23.6.0 for `@simpletoolsindiaorg/gondolin`, plus QEMU (requires installation through your package manager).
...
RUN npm install -g --ignore-scripts @simpletoolsindiaorg/ai-coding-agent
