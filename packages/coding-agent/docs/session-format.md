...
```
~/.ai/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.ai/agent/sessions/`.

Ai also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, ai uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([ai](https://github.com/simpletoolsindiaorg/ai)):
