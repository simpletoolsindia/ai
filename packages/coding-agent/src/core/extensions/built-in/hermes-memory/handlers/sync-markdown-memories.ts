...
import { parseMarkdownMemoryEntry, syncMemoryEntry } from "../store/sqlite-memory-store.ts";
...
export function syncMarkdownMemoriesToSqlite(
	...
	const importFile = (filePath: string, target: "memory" | "user" | "failure", project: string | null = null) => {
		if (!fs.existsSync(filePath)) return;
