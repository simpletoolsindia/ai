...
import type {
	...
	AgentTool,
	ThinkingLevel,
} from "@simpletoolsindiaorg/ai-agent";
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@simpletoolsindiaorg/ai-provider";
import {
	clampThinkingLevel,
...
} from "./export-transcript/index.ts";
import {
	type ContextUsage,
...
export class AgentSession {
	 ...
	 * - Handles extension commands (registered via ai.registerCommand) immediately, even during streaming
	 ...
	 * Clean readable transcript — suitable for pasting into docs, wikis, or chat.
	 * @param options Output path and content filters
	 * @returns The resolved output file path.
	 */
	exportToMarkdown(options?: MarkdownExportOptions | string): string {
		return exportSessionToMarkdown(this.sessionManager, this.state, options);
	}

	/**
	 * Export the current session branch to a structured JSON file.
	 * @param options Output path and JSON formatting
	 * @returns The resolved output file path.
	 */
	exportToJson(options?: JsonExportOptions | string): string {
		return exportSessionToJson(this.sessionManager, this.state, options);
	}
