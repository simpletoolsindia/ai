> ai can help you use the SDK. Ask it to build an integration for your use case.
...
```bash
npm install @simpletoolsindiaorg/ai-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@simpletoolsindiaorg/ai-coding-agent";
...
The `Agent` class (from `@simpletoolsindiaorg/ai-agent`) handles the core LLM interaction. Access it via `session.agent`.
...
- Project extensions (`.ai/extensions/`)
- Project skills:
  - `.ai/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.ai/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.ai/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@simpletoolsindiaorg/ai-provider";
import { AuthStorage, ModelRegistry } from "@simpletoolsindiaorg/ai-coding-agent";
...
// Default: uses ~/.ai/agent/auth.json and ~/.ai/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@simpletoolsindiaorg/ai-coding-agent";
...
```typescript
import { createAgentSession } from "@simpletoolsindiaorg/ai-coding-agent";
...
import { createAgentSession, defineTool } from "@simpletoolsindiaorg/ai-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.ai/agent/extensions/`, `.ai/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@simpletoolsindiaorg/ai-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@simpletoolsindiaorg/ai-coding-agent";
...
1. Global: `~/.ai/agent/settings.json`
2. Project: `<cwd>/.ai/settings.json`
