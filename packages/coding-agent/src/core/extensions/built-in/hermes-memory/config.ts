...
import { normalizeConfiguredMemoryDir, normalizeProjectsMemoryDir } from "./paths.ts";
import type { MemoryConfig, MemoryOverflowStrategy, SessionSearchVariant, ThinkingLevel } from "./types.ts";
...
export const DEFAULT_CONFIG_PATH = path.join(os.homedir(), ".pi", "agent", "hermes-memory-config.json");
