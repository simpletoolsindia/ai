...
export function createWebsearchToolDefinition(
	...
	return {
		...
		label: "Websearch",
		description: [
			"Search the public web via SearXNG and return the top results as a markdown list.",
			"- Each result includes title, URL, and snippet. Use webfetch to read a specific result's content.",
			"- Only the configured SearXNG instance is queried (no other engines, no redirects to alternative search providers).",
			'- Safe search defaults to "0" (none). Use the language param to filter by language.',
			"- Results are not authoritative; cross-check before acting on them.",
		].join(" "),
