...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @simpletoolsindiaorg/ai-provider <command> [provider]
...
Examples:
  ...
  npx @simpletoolsindiaorg/ai-provider list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @simpletoolsindiaorg/ai-provider list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @simpletoolsindiaorg/ai-provider --help' for usage`);
	process.exit(1);
}
