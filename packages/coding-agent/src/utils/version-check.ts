import { getAiUserAgent } from "./ai-user-agent.ts";

const LATEST_VERSION_URL = "https://api.simpletoolsindiaorg.invalid/latest-version";
const DEFAULT_VERSION_CHECK_TIMEOUT_MS = 10000;

export interface LatestAiRelease {
	version: string;
	packageName?: string;
	note?: string;
}
...
export async function getLatestAiRelease(
	...
	const response = await fetch(LATEST_VERSION_URL, {
		headers: {
			"User-Agent": getAiUserAgent(currentVersion),
...
export async function getLatestAiVersion(
	currentVersion: string,
	options: { timeoutMs?: number } = {},
