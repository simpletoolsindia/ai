...
export async function execChildPrompt(
	...
	try {
		const result = (await pi.exec("ai", buildChildPiPromptArgs(prompt, config), execOptions)) as PiExecResult;
		if (
			result.code === 0 ||
	...
	return pi.exec("ai", basePromptArgs(prompt), execOptions) as Promise<PiExecResult>;
}
