 ...
 *
 *   mkdir -p ~/.ai/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.ai/agent/extensions/
 *
 * Or:
 *
 *   ai -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .ai, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@simpletoolsindiaorg/ai-coding-agent";
...
export default function (ai: ExtensionAPI) {
	...
	ai.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
