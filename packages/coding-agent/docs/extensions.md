> ai can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend pi's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.ai/agent/extensions/` (global) or `.ai/extensions/` (project-local) for auto-discovery. Use `ai -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.ai/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.ai/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.ai/agent/extensions/*.ts` | Global (all projects) |
| `~/.ai/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.ai/extensions/*.ts` | Project-local |
| `.ai/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@simpletoolsindiaorg/ai-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@simpletoolsindiaorg/ai-provider` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@simpletoolsindiaorg/ai-tui` | TUI components for custom rendering |
...
export default function (pi: ExtensionAPI) {
  // Subscribe to events
 ai.on("event_name", async (event, ctx) => {
...
export default async function (pi: ExtensionAPI) {
 ...
 ai.registerProvider("local-openai", {
...
```typescript
ai.on("project_trust", async (event, ctx) => {
...
```typescript
ai.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
ai.on("session_start", async (event, ctx) => {
...
```typescript
ai.on("session_before_switch", async (event, ctx) => {
...
```typescript
ai.on("session_before_fork", async (event, ctx) => {
...
```typescript
ai.on("session_before_compact", async (event, ctx) => {
...
ai.on("session_compact", async (event, ctx) => {
...
```typescript
ai.on("session_before_tree", async (event, ctx) => {
...
ai.on("session_tree", async (event, ctx) => {
...
```typescript
ai.on("session_shutdown", async (event, ctx) => {
...
```typescript
ai.on("before_agent_start", async (event, ctx) => {
...
ai.on("agent_end", async (event, ctx) => {
...
```typescript
ai.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

ai.on("turn_end", async (event, ctx) => {
...
```typescript
ai.on("message_start", async (event, ctx) => {
  // event.message
});

ai.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

ai.on("message_end", async (event, ctx) => {
...
```typescript
ai.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

ai.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

ai.on("tool_execution_end", async (event, ctx) => {
...
```typescript
ai.on("before_provider_request", (event, ctx) => {
...
```typescript
ai.on("after_provider_response", (event, ctx) => {
...
```typescript
ai.on("model_select", async (event, ctx) => {
...
```typescript
ai.on("thinking_level_select", async (event, ctx) => {
...
```typescript
import { isToolCallEventType } from "@simpletoolsindiaorg/ai-coding-agent";
...
```typescript
import { isBashToolResult } from "@simpletoolsindiaorg/ai-coding-agent";

ai.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@simpletoolsindiaorg/ai-coding-agent";

ai.on("user_bash", (event, ctx) => {
...
```typescript
ai.on("before_agent_start", (event, ctx) => {
...
```typescript
import { SessionManager } from "@simpletoolsindiaorg/ai-coding-agent";

 ai.registerCommand("switch", {
...
```typescript
 ai.registerCommand("reload-runtime", {
...
export default function (pi: ExtensionAPI) {
 ...
 ai.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
 ai.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@simpletoolsindiaorg/ai-provider";
...
```typescript
// Simple text message
 ai.sendUserMessage("What is 2+2?");
...
// During streaming - must specify delivery mode
 ai.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
 ai.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
```typescript
import type { AutocompleteItem } from "@simpletoolsindiaorg/ai-tui";

 ai.registerCommand("deploy", {
...
// Override baseUrl for an existing provider (keeps all models)
 ai.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
 ai.registerProvider("corporate-ai", {
...
```typescript
 ai.registerCommand("my-setup-teardown", {
...
```typescript
import { withFileMutationQueue } from "@simpletoolsindiaorg/ai-coding-agent";
...
**Important:** Use `StringEnum` from `@simpletoolsindiaorg/ai-provider` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [read.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@simpletoolsindiaorg/ai-coding-agent";
...
export default function (pi: ExtensionAPI) {
 ...
 ai.registerTool({ name: "db_connect", ... });
 ai.registerTool({ name: "db_query", ... });
 ai.registerTool({ name: "db_close", ... });

 ai.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/simpletoolsindiaorg/ai/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@simpletoolsindiaorg/ai-coding-agent";
...
```typescript
import { Text, Component } from "@simpletoolsindiaorg/ai-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@simpletoolsindiaorg/ai-coding-agent";
import { matchesKey } from "@simpletoolsindiaorg/ai-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@simpletoolsindiaorg/ai-coding-agent";
