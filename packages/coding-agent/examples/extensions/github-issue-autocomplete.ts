...
async function resolveGitHubRepo(ai: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await ai.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (ai: ExtensionAPI): void {
	ai.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(ai, ctx.cwd);
		if (!resolvedRepo.ok) {
