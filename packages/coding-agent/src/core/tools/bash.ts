...
import { Container, Text, truncateToWidth } from "@simpletoolsindiaorg/ai-tui";
...
/**
 * Create bash operations using ai's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want ai's
...
- When issuing multiple commands, prefer parallel tool calls in a single message for independent commands. For sequential dependent commands, chain with '&&'.`,
		...
		async execute(
			...
			const output = new OutputAccumulator({ tempFilePrefix: "ai-bash" });
