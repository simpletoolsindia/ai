...
async function checkDirtyRepo(
	...
	const { stdout, code } = await ai.exec("git", ["status", "--porcelain"]);
...
export default function (ai: ExtensionAPI) {
	ai.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(ai, ctx, action);
	});

	ai.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(ai, ctx, "fork");
	});
}
