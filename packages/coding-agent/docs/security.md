...
When an interactive session starts in a project with trust inputs and no saved decision, pi asks whether to trust the project. Saved decisions are stored per canonical working directory in `~/.ai/agent/trust.json`.
...
To report a security issue, follow the repository [Security Policy](https://github.com/simpletoolsindiaorg/ai/blob/main/SECURITY.md). Do not open a public issue for security-sensitive reports.
