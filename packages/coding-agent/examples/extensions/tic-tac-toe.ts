...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "@simpletoolsindiaorg/ai-coding-agent";
import { StringEnum } from "@simpletoolsindiaorg/ai-provider";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@simpletoolsindiaorg/ai-tui";
...
export default function (ai: ExtensionAPI) {
	ai.on("session_start", async (_event, ctx) => reconstructState(ctx));
	ai.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	ai.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	ai.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	ai.registerCommand("tic-tac-toe", {
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						ai.appendEntry(SAVE_TYPE, getBoardDetails());
