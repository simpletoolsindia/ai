...
Example extensions for ai-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
ai --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.ai/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `ai.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs ai theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `ai.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using ai-provider's built-in Anthropic/OpenAI streaming via proxy |
