...
export function createSubagentToolDefinition(
	...
	return {
		...
		label: "Subagent",
		description:
			"Delegate focused investigation to a specialized subagent with its own context window. Subagents are read-only. Use for deep investigation without polluting main context.",
		promptSnippet: "Delegate to sub-agent (single, parallel, or chain mode)",
		promptGuidelines: [
			"Use subagent for separable sub-parts or deep investigation",
			"Do NOT use for simple file reads, greps, or finds",
			"Do NOT use for tasks that need to write or modify files",
			"Single: { agent, task } — one subagent",
			"Parallel: { tasks: [{agent, task}, ...] } — up to 8 in parallel",
