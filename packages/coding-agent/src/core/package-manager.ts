...
function isOfflineModeEnabled(): boolean {
	const value = process.env.AI_OFFLINE;
...
type SkillDiscoveryMode = "ai" | "pi" | "agents";
...
function readAiManifestFile(packageJsonPath: string): AiManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { ai?: AiManifest; pi?: AiManifest };
		return pkg.ai ?? pkg.pi ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readAiManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "ai");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside ai and resolve ai APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @simpletoolsindiaorg/ai-* peers. Stale auto-installed ai peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "ai-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = this.readAiManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof AiManifest];
	...
	private readAiManifest(packageRoot: string): AiManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			...
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "ai"),
				projectMetadata,
		...
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "ai"),
			userMetadata,
