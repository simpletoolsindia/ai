 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit ai cleanly.
 */
...
export default function (ai: ExtensionAPI) {
	// Register a /quit command that cleanly exits ai
	ai.registerCommand("quit", {
		description: "Exit ai cleanly",
	...
	ai.registerTool({
		...
		description: "Complete a task and exit ai",
	...
	ai.registerTool({
		...
		description: "Deploy the application and exit ai",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await ai.exec("npm", ["run", "deploy", params.environment], { signal });
