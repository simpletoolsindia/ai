...
import type { Component, EditorTheme, TUI } from "@simpletoolsindiaorg/ai-tui";
import { truncateToWidth, visibleWidth } from "@simpletoolsindiaorg/ai-tui";
...
export default function (ai: ExtensionAPI) {
	...
	ai.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await ai.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = ai.getThinkingLevel();
