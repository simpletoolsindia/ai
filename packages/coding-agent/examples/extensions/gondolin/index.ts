 ...
 *
 * Runs ai's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   ai -e /path/to/ai/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (ai: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `ai ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	ai.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
