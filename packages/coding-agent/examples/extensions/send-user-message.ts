 ...
 *
 * Demonstrates ai.sendUserMessage() for sending user messages from extensions.
 * Unlike ai.sendMessage() which sends custom messages, sendUserMessage() sends
	...
	ai.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				ai.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	ai.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				ai.sendUserMessage(args, { deliverAs: "followUp" });
