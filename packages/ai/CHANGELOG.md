...
- Added Ollama Cloud as a built-in OpenAI-compatible provider. Models are sourced from `https://ollama.com/v1/models` (validated against models.dev metadata) and shipped with sensible defaults: `supportsStore: false`, `supportsDeveloperRole: false`, `supportsReasoningEffort: false`, `supportsLongCacheRetention: false`. Authenticate with the `OLLAMA_API_KEY` environment variable or the `ollama-cloud` key in `~/.pi/agent/auth.json`.
