> ai can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@simpletoolsindiaorg/ai-tui`](https://github.com/simpletoolsindiaorg/ai/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@simpletoolsindiaorg/ai-tui";
...
```typescript
import { Container, type Focusable, Input } from "@simpletoolsindiaorg/ai-tui";
...
**In custom tools** via `ai.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = ai.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@simpletoolsindiaorg/ai-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@simpletoolsindiaorg/ai-tui";
```
...
```typescript
import { matchesKey, Key } from "@simpletoolsindiaorg/ai-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@simpletoolsindiaorg/ai-tui";
...
```typescript
import { getMarkdownTheme } from "@simpletoolsindiaorg/ai-coding-agent";
import { Markdown } from "@simpletoolsindiaorg/ai-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@simpletoolsindiaorg/ai-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@simpletoolsindiaorg/ai-coding-agent";
import { DynamicBorder } from "@simpletoolsindiaorg/ai-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@simpletoolsindiaorg/ai-tui";
...
```typescript
import { BorderedLoader } from "@simpletoolsindiaorg/ai-coding-agent";

 ai.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@simpletoolsindiaorg/ai-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@simpletoolsindiaorg/ai-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@simpletoolsindiaorg/ai-tui";

 ai.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while ai is streaming a response.
...
import { matchesKey, truncateToWidth } from "@simpletoolsindiaorg/ai-tui";
