...
| `/export [file]` | Export session. Format by extension: `.html` (default), `.md` (markdown), `.json` (structured), `.jsonl` (raw). No path = write to default location. |
...
Sessions are saved automatically to `~/.ai/agent/sessions/`, organized by working directory.
...
- `~/.ai/agent/AGENTS.md` for global instructions
...
- `~/.ai/agent/SYSTEM.md` globally

Append to the default prompt without replacing it with `APPEND_SYSTEM.md` in either location.

### Project Trust

On interactive startup, pi asks before trusting a project folder that contains project-local inputs and has no saved decision in `~/.ai/agent/trust.json`. Trusting a project allows pi to read project instructions (`AGENTS.md`/`CLAUDE.md`), load `.ai/settings.json` and `.pi` resources, install missing project packages, and execute project extensions.
...
Use `/trust` in interactive mode to save a project trust decision for future sessions. It writes `~/.ai/agent/trust.json` only; the current session is not reloaded, so restart pi for changes to take effect.

## Exporting and Sharing Sessions

Use `/export [file]` to write a session. The format is determined by the file extension (`.html`, `.md`, `.json`, `.jsonl`). HTML is the default if no extension matches.
...
| `--export <in> [out]` | Export a session file to HTML |
