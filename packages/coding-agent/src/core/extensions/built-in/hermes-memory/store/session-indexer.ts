...
import { getSessionFiles, type ParsedSession, parseSessionFile } from "./session-parser.js";
...
export function indexSession(dbManager: DatabaseManager, session: ParsedSession): IndexResult {
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(session.id, session.project, session.cwd, session.startedAt, session.endedAt, session.messages.length);
