import { Markdown, type MarkdownTheme } from "@simpletoolsindiaorg/ai-tui";
...
import { getLatestAiRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
Options:
  -l, --local       Install project-locally (.ai/settings.json)
...
Options:
  -l, --local       Remove from project settings (.ai/settings.json)
  -a, --approve     Trust project-local files for this command
  -na, --no-approve Ignore project-local files for this command
...
Update ai and installed packages.
...
Options:
  ...
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall ai even if the current version is latest

Short forms:
  ${APP_NAME} update                Update ai and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update ai             Update ai only (self works as alias to ai)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "ai" || source === "pi";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(npmCommand?: string[], updatePackageName = PACKAGE_NAME): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of ai executable: ${entrypoint}`);
	}
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestAiRelease(VERSION);
