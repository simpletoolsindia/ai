> ai can create themes. Ask it to build one for your setup.
...
- Built-in: `dark`, `light`
- Global: `~/.ai/agent/themes/*.json`
- Project: `.ai/themes/*.json` (only after the project is trusted)
...
```bash
mkdir -p ~/.ai/agent/themes
vim ~/.ai/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/simpletoolsindiaorg/ai/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
