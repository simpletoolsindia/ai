 ...
 * - ~/.ai/agent/presets.json (global)
 * - <cwd>/.ai/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `ai --preset plan` - start with plan preset
...
import { DynamicBorder, getAgentDir } from "@simpletoolsindiaorg/ai-coding-agent";
import type { Api, Model } from "@simpletoolsindiaorg/ai-provider";
import { Container, Key, type SelectItem, SelectList, Text } from "@simpletoolsindiaorg/ai-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".ai", "presets.json");
...
export default function presetExtension(ai: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: ai.getThinkingLevel(),
				tools: ai.getActiveTools(),
			};
		}
		...
		if (preset.provider && preset.model) {
			...
			if (model) {
				const success = await ai.setModel(model);
				if (!success) {
		...
		if (preset.thinkingLevel) {
			ai.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = ai.getAllTools().map((t) => t.name);
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.ai/agent/presets.json or .ai/presets.json", "warning");
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await ai.setModel(originalState.model);
				}
				ai.setThinkingLevel(originalState.thinkingLevel);
				ai.setActiveTools(originalState.tools);
			} else {
				ai.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	ai.registerShortcut(Key.ctrlShift("u"), {
	...
	ai.on("before_agent_start", async (event) => {
	...
	ai.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = ai.getFlag("preset");
	...
	ai.on("turn_start", async () => {
		if (activePresetName) {
			ai.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
