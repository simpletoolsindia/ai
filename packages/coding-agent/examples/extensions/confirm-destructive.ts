...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@simpletoolsindiaorg/ai-coding-agent";

export default function (ai: ExtensionAPI) {
	ai.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
