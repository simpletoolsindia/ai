...
import { type ImageContent, modelsAreEqual } from "@simpletoolsindiaorg/ai-provider";
import { ProcessTerminal, setKeybindings, TUI } from "@simpletoolsindiaorg/ai-tui";
...
function formatProjectTrustPrompt(cwd: string): string {
	return `Trust project folder?\n${cwd}\n\nThis allows ai to read project instructions (AGENTS.md/CLAUDE.md), load .ai settings and resources, install missing project packages, and execute project extensions.`;
}
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.AI_OFFLINE);
	if (offlineMode) {
