...
export default function (ai: ExtensionAPI) {
	ai.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await ai.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[ai] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await ai.exec("git", ["add", "-A"]);
		const { code: commitCode } = await ai.exec("git", ["commit", "-m", commitMessage]);
