...
import { COMBINED_REVIEW_PROMPT } from "../constants.ts";
...
export function setupBackgroundReview(
	...
	pi.on("turn_end", async (event, ctx) => {
		...
		if (currentProject !== null) {
			reviewPrompt.push("", "--- Current Project Memory ---", currentProject || "(empty)");
		}

		reviewPrompt.push("", "--- Conversation to Review ---", parts.join("\n\n"));
